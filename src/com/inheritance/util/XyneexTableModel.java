...
/**
 * A TableModel that better supports the processing of rows of data. That is, the data is treated more like a row than an individual cell. Hopefully this class can be used as a parent class instead of
 * extending the AbstractTableModel when you need custom models that contain row related data.
 *
 * A few methods have also been added to make it easier to customize properties of the model, such as the column class and column editability.
 *
 * Any class that extends this class must make sure to invoke the setRowClass() and setDataAndColumnNames() methods either directly, by using the various constructors, or indirectly.
 *
 * @param <T> The data type that will be specified to represent each row.
 */
public abstract class XyneexTableModel<T> extends AbstractTableModel
{
    /**
     * The data that will be contained in this model.
     */
    ...
    /**
     * Editable columns will be specified here, the column indexes must match this array's indexes.
     */
    protected Boolean[] isColumnEditable;

    /**
     * The data type for each row.
     */
    private Class rowClass = Object.class;

    /**
     * Specifies if model is editable.
     */
     ...
     */
    protected XyneexTableModel(Class rowClass)
    {
     ...
     *
     * Each column's name will be taken from the <code>columnNames</code> List and the number of colums is determined by thenumber of items in the <code>columnNames</code> List.
     *
     ...
     */
    protected XyneexTableModel(List<String> columnNames)
    {
     ...
     *
     * Each item in the <code>modelData</code> List must also be a List Object containing items for each column of the row.
     *
     * Each column's name will be taken from the <code>columnNames</code> List and the number of colums is determined by thenumber of items in the <code>columnNames</code> List.
     *
     * Sub classes creating a model using this constructor must make sure to invoke the setRowClass() method.
     ...
     * @param columnNames	   <code>List</code> containing the names of the new columns
     */
    protected XyneexTableModel(List<T> modelData, List<String> columnNames)
    {
     ...
     * @param rowClass the class of row data to be added to the model
     */
    protected XyneexTableModel(List<T> modelData, List<String> columnNames, Class rowClass)
    {
        this.setDataAndColumnNames(modelData, columnNames);
        this.setRowClass(rowClass);
    }
...
//
     ...
     *
     * @return a name for this column using the string value of the appropriate member in <code>columnNames</code>. If <code>columnNames</code> does not have an entry for this index then the default
     * name provided by the superclass is returned
     */
    @Override
...
//
    ...
    {
        this.insertRow(getRowCount(), rowData);
    }
     ...
     *
     * @param rows the list/array of row indexes requested.
     * @return an array of Objects for the requested rows.
     */
    @SuppressWarnings("unchecked")
    public T[] getRowsAsArray(int... rows)
    {
        List<T> rowData = this.getRowsAsList(rows);
        T[] array = (T[])Array.newInstance(this.rowClass, rowData.size());
