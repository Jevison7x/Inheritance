 ...
 * @author Jevison7x
 */
public class ResultsTableModel extends XyneexTableModel<Object[]>
{
    ...
    protected static final String BLOCKED_REASON = "Blocked Reason";

    public ResultsTableModel(List<Heir> heirs)
    {
    ...
    private void setColumnNames()
    {
        ...
        {
            SN, HEIR_NAME, HEIR_TYPE, SHARE, BLOCKED_REASON
        };
    }

    private void setData(List<Heir> heirs)
    {
        ...
        for(Heir heir : heirs)
        {
            ...
            row[this.findColumn(HEIR_NAME)] = heir.getName();
            row[this.findColumn(HEIR_TYPE)] = heir.getHeirType();
            row[this.findColumn(SHARE)] = heir.getInheritedValue();
            row[this.findColumn(BLOCKED_REASON)] = heir.getBlockReason();
            this.data.add(row);
        }
    }
    ...
    {
        this.canEdit = new boolean[data.size()][this.childColumnNames.length];
